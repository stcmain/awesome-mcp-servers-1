...
- <img src="https://github.com/favicon.ico" height="14" /> [stcmain/whats-loaded-mcp](https://github.com/stcmain/whats-loaded-mcp) - Shows what is consuming an agent's context window before a session starts: always-loaded skill descriptions ranked by token cost, skills installed twice under different names, CLAUDE.md/AGENTS.md sizes including what their @import lines pull in, and MCP servers configured across clients. Read-only, no child processes, no network.
